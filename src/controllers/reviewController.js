...
const createReview =  async (req, res) => {
 ...
 if(!mongoose.isValidObjectId(bookId) ){
    ...
    let  book = await booksModel.findOne({ _id: bookId, isDeleted: false })    //not exists it would give error
    ...
    await book.save()
    book = book.toObject()
    const reviewer = await reviewModel.create({ bookId, reviewedBy,reviewedAt : Date.now(), rating, review } )
...
const Allreviewer = await  reviewModel.find({bookId,isDeleted : false})
